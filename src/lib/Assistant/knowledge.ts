...
export const KNOWLEDGE: KnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      ...
      "• Getting in — one person starts a room and becomes the host; their browser is the room, and the film, voices and chat go straight between devices. The first invite is ready as soon as the room opens (People → Share, Copy or QR; one invite per guest). With “Anywhere”, the guest opens it and taps Join, and the host's invite card asks “<name> wants to join” — one tap on Let them in and they're watching; nothing is copied back. That works because the guest's reply passes once through a public relay, encrypted with a key that exists only in the invite link, so the relay can't read it. If no relay can be reached, the guest is shown the reply to send back and the host pastes it. “This network only” contacts nothing outside the Wi-Fi, so there the reply always goes back by hand; “Anywhere” also asks a public STUN server for the device's address\n" +
      ...
      "• Speakers & headphones — on the Watch tab, play the room's sound (film and voices) on several audio outputs of one device at once, such as two pairs of Bluetooth headphones, each tuned on its own: volume, mute, a live level meter, a name (“Mum's headphones”), a delay of up to 800 ms to line slower Bluetooth devices up with the rest (Sync check plays three clicks on every output at once; the one heard first wants more delay), stereo, mono, left-only or right-only, film + voices, film only or voices only, Clear voices (lifts dialogue) and Night mode (evens out loud and quiet); Test plays a chime to tell them apart, and the device's own speaker can be silenced. Pair them in the system's Bluetooth settings first; the browser asks for the mic once only to show their names. Sound only — Bluetooth can't carry the picture, so another screen joins by invite. Works in Chrome and Edge on a computer; YouTube's sound can't be moved off the system output\n" +
