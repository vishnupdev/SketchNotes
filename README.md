...
| Agenda | [`lib/schedule.ts`](./src/lib/schedule.ts) | Todos' fifth view answers "what is coming" by merging **tasks and reminders**, grouped by distance from now rather than by calendar unit. The merge is shell-level for the same reason [`storage-keys.ts`](./src/lib/storage-keys.ts) is — it has to know about two apps precisely because neither may know about the other. Dependency direction is one-way: this imports both apps' types, neither imports this, so a third source is a new adapter and nothing else. A repeating reminder that has fired is *upcoming*, not done. |
| One camera | [`lib/camera.ts`](./src/lib/camera.ts) | Four apps open cameras and three had grown their own copy of the support probe, the `getUserMedia` error mapping and the track teardown. Shared now — three copies of a security-visible release path was two too many. Constraints and wording stay per-app on purpose: a document scanner wants every sensor pixel and cannot say "attach a photo instead", where Color Lens wants a 1080p preview and can. |
| Vault | [`components/Vault/`](./src/components/Vault) | TOTP from first principles on `crypto.subtle` ([`lib/Vault/totp.ts`](./src/lib/Vault/totp.ts), checked against the RFC 6238 vectors for SHA-1/256/512), so codes need no network. The vault is **one** AES-GCM blob under a 600k-round PBKDF2 key ([`lib/Vault/crypto.ts`](./src/lib/Vault/crypto.ts)) — one blob rather than one per entry, so the *number* of entries and the length of their names leak nothing. The key lives in a module variable, never in the store, so it cannot be walked by devtools or serialised; unmounting the app drops it. An account can also be handed **back out** as the `otpauth://` QR it arrived as — via the shared [`lib/qr/encode.ts`](./src/lib/qr/encode.ts), so it scans as well as the original — which is what keeps a vault in a browser from being a one-way trip; behind an explicit reveal, because the code *is* the secret and a photograph of it is permanent. Strength is computed from the generator's alphabet and length, never guessed from the output — the one place it *has* to guess (the user's own passphrase) reads deliberately low. |
| Sheets | [`components/Sheets/`](./src/components/Sheets) | A real RFC 4180 scanner ([`lib/Sheets/csv.ts`](./src/lib/Sheets/csv.ts)): quoted delimiters, quoted **newlines**, doubled quotes, BOM, CRLF, sniffed semicolon/tab/pipe. Every failure mode here yields a *plausible* table rather than an error, which is why the suite is 58 tests. A column is typed only if every value fits, and ambiguous dates (`03/04/2024`) stay text rather than being guessed at a locale. Sorting and filtering are an **ordering over rows**, never a mutation, so an edit lands on the right record whatever the view — and the bar chart's scale always includes zero, because bar length *is* the value. |
| Clip | [`components/Clip/`](./src/components/Clip) | Screen + camera is **composited onto a canvas** and the canvas recorded — `MediaRecorder` takes one stream, and two video tracks produce a file whose second track most players silently drop. Single-source recording skips the canvas entirely. The container is picked by walking a preference list against `isTypeSupported` ([`lib/Clip/recorder.ts`](./src/lib/Clip/recorder.ts)), because Chromium records WebM, Safari records MP4, and the wrong ask yields a file that appears to record and won't play. Clips stay in memory: the origin's quota is shared with every other app here, and a minute of 1080p would spend it. The countdown runs **after** the permission prompt, never before — counting down first puts the browser's own dialog in the opening seconds of the take — and the level meter runs during it, off the shared [`lib/audio-level.ts`](./src/lib/audio-level.ts), because a ten-minute recording against a muted mic is this app's most expensive failure and is invisible without one. |
| Cards | [`components/Cards/`](./src/components/Cards) | SM-2 as a **pure function** of card, grade and moment ([`lib/Cards/srs.ts`](./src/lib/Cards/srs.ts)) — which is the only way a scheduler is testable, since using it to find out it's wrong takes a month. Ease is clamped so a run of failures can't strand a card at one day forever; a lapse keeps its interval rather than zeroing it. The queue is recomputed after every answer, so a card just failed genuinely returns in the same sitting. Import picks the separator that splits the most lines cleanly, and *reports* what it couldn't split. |
| Exif | [`components/Exif/`](./src/components/Exif) | The strip is a **segment edit, not a re-encode** ([`lib/Exif/containers.ts`](./src/lib/Exif/containers.ts)): JPEG APP1/APP13/COM, PNG text/time/eXIf and WebP EXIF/XMP chunks are dropped and every other byte copied through, so the image data is bit-identical — where the usual canvas round-trip costs a generation of JPEG quality and the ICC profile. WebP also gets its VP8X flags cleared and RIFF size rewritten, or the file claims chunks it no longer has. The TIFF parser ([`lib/Exif/tiff.ts`](./src/lib/Exif/tiff.ts)) follows offsets from the *TIFF header* — the detail naive readers get wrong — and stores nothing at all. [`batch.ts`](./src/lib/Exif/batch.ts) does a folder at a time, sequentially rather than concurrently (sixty phone photos held at once is a gigabyte and a dead tab), numbering duplicate zip entries so none silently overwrites another, and leading with how many of the batch carried a location. |
| Calc | [`components/Calc/`](./src/components/Calc) | `%` is **contextual**, as on a phone and not as in a programming language: a percent literal carries a flag through evaluation ([`lib/Calc/expression.ts`](./src/lib/Calc/expression.ts)) that only `+` and `−` consult, so `1250 + 18%` is 1475 while `18%` alone is 0.18. Modulo is therefore the word `mod`, and implicit multiplication is refused after a percent so `17 % 5` errors rather than quietly returning 0.85 — the silent wrong answer is the one worth engineering against. The tape ([`lib/Calc/tape.ts`](./src/lib/Calc/tape.ts)) evaluates line by line with only the names above in scope, so a cycle is impossible rather than detected, and **one bad line never costs you the others**. Answers print back per line instead of in a gutter, which misaligns silently on the first soft-wrapped line. Bases ([`lib/Calc/bases.ts`](./src/lib/Calc/bases.ts)) is all `BigInt` masked to a chosen width, because JS's bitwise operators coerce to 32-bit signed and cannot express a 64-bit mask at all. |
...
| Metronome | [`components/Metronome/`](./src/components/Metronome) | A **lookahead scheduler** ([`lib/Metronome/engine.ts`](./src/lib/Metronome/engine.ts)): a 25 ms timer only *books* clicks on the audio clock, so a busy main thread delays the booking, never the click. The plan is re-read per click, so a tempo nudge lands on the next one. Tap tempo takes the **median** gap, so one fumbled tap doesn't skew it; accents default to how the metre is felt (6/8 in threes, 7/8 as 2+2+3). Ramp and silent-bar arithmetic is pure and tested ([`lib/Metronome/rhythm.ts`](./src/lib/Metronome/rhythm.ts)). |
| Breathe | [`components/Breathe/`](./src/components/Breathe) | The orb is **one clock, not an animation**: each frame, elapsed seconds → phase → eased lung level ([`lib/Breathe/patterns.ts`](./src/lib/Breathe/patterns.ts)), written as `--level`/`--p`/`--spin` on the stage; every layer (orb, halo rings, particles in `cqw`, glow, token-mixed colour, progress sweep) is CSS from those, so a frame is a style update and a composite, never a React render. `--level` is an `@property` number, which is what lets the idle pulse be a pure keyframe. Sessions round to whole cycles so they end on an exhale; reduced motion keeps the meaning (words, countdown, ring, colour) and drops the movement. |
| Watch Party | [`components/WatchParty/`](./src/components/WatchParty) | A **star, not a mesh**: the host's browser is the room and each guest pairs with it by the same hand-carried codes File Drop uses ([`lib/rtc/`](./src/lib/rtc)), so there is still no server. Every player steers toward one shared line — position, rate and the host time it was true at ([`lib/WatchParty/sync.ts`](./src/lib/WatchParty/sync.ts)) — against a clock offset measured NTP-style; a small drift is closed by playing ±10% for a few seconds, a big one by a seek, and a *host* that buffers moves the line instead so the room waits. Media lines are added after joining by renegotiating over the data channel, which keeps the invite one QR long; voice is a **mix-minus** built on the host, so each guest takes one voice track whatever the room size ([`lib/WatchParty/voice.ts`](./src/lib/WatchParty/voice.ts)). Every guest message is validated before the host acts on it ([`lib/WatchParty/protocol.ts`](./src/lib/WatchParty/protocol.ts)). |
